...
import 'package:crypto_trade/features/trades/presentation/widgets/fiat_balance_card.dart';
import 'package:crypto_trade/features/trades/presentation/widgets/fiat_payment_method_selector.dart';
import 'package:crypto_trade/features/trades/presentation/widgets/fiat_quick_select_grid.dart';
...
class TradeFiatTabBody extends StatelessWidget {
  const TradeFiatTabBody({super.key});
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        verticalSpace(12),
        CustomTextFormField(
          controller: context.read<FiatCubit>().amountController,
    ...
    return BlocConsumer<FiatCubit, FiatState>(
      ...
      listener: (context, state) {
        if (state.fiatStatus == FiatStatus.success) {
          context.read<WalletCubit>().depositFiat(state.fiatDepositAmount);
